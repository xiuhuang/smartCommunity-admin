...
@connect(
  ({
    ...
    resident,
    loading: loading.models.resident,
  }),
)
class Resident extends Component<BuildingsProps> {
  state = {
    selectedRows: [],
    visible: false,
    record: '',
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      if (dispatch) {
        dispatch({
          ...
          callback: (res: any) => {
            downloadExcelForPost(res, '居民认证.xlsx');
          },
        });
      }
    });
  ...
  render() {
    const { loading, resident } = this.props;
...
export default Form.create<BuildingsProps>()(Resident);
