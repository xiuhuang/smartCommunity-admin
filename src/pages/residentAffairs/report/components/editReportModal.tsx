...
class FormList extends Component<FormListProps> {
  ...
  componentWillReceiveProps(nextProps: any) {
    ...
    if (!visible && nextProps.visible && nextProps.repairId) {
      this.getDetail(nextProps.repairId);
  ...
  submit() {
    const { form, dispatch, getData, handleVisible, repairId } = this.props;
    form.validateFields((err, fieldsValue) => {
      if (err) return;

      fieldsValue.businessId = repairId;
      if (dispatch) {
        dispatch({
    ...
    return (
      ...
      >
        <Form className={`${styles.formBox} form12`}>
          ...
          <FormItem label="预约时间" labelCol={{ span: 6 }} wrapperCol={{ span: 14 }}>
